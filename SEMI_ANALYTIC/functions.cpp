...
void update_TPM(long int N_states, double T, long int index1, long int index2, double diff, std::vector<std::vector<double>> &TPM){
	...
	if(diff < 0.){
		...
		TPM[index1][index2] = TPM[index1][index2]/N_states;  // scale by prob of drawing a given state (metropolis algo)
	}else{
		TPM[index1][index2] = std::exp(-diff/T); // sometimes accept higher energy state due to thermal fluctuations
		TPM[index1][index2] = TPM[index1][index2]/N_states;  // scale by prob of drawing a given state (metropolis algo)s
	}
